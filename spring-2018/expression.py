    ...
    for ch in formula:
        if not ch in OPERATORS:
            ...
            while (
                op_stack
                and op_stack[-1] != "("
                and PRIORITY[ch] <= PRIORITY[op_stack[-1]]
            ):
                op = op_stack.pop()
                a = exp_stack.pop()
                b = exp_stack.pop()
...
def evaluate_postfix(formula):
    ...
    for ch in formula:
        if ch not in OPERATORS:
            ...
            if str(a).upper() in ALPHABETS:
                a = ALPHABETS.index(a) + 1

            if str(b).upper() in ALPHABETS:
                b = ALPHABETS.index(b) + 1

            a, b = map(float, [a, b])

            stack.append(do_math(ch, a, b))

    val = stack[-1]
    if str(val).upper() in ALPHABETS:
        val = ALPHABETS.index(val) + 1
        ...
        for i in range(len(exps) - 2):
            if exps[i] in OPERATORS:
                if not exps[i + 1] in OPERATORS and not exps[i + 2] in OPERATORS:
                    ...
                    a, b = map(float, [a, b])
                    exps = exps[:i] + [do_math(op, a, b)] + exps[i + 3:]
                    break
...
if __name__ == "__main__":
    ...
    # infix_to_prefix("4+3*5")
    # infix_to_postfix("4+3*5")
    # evaluate_infix("4+3*5")
    # evaluate_infix('1+(3+4*6+6*1)*2/3')
